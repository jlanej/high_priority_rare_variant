...
RUN set -eux; \
    ...
    # ...including the four score plugins Step 2 runs, the stock NMD plugin (no data file — it is
    # what Step 9's V5 rung rests on, so a base image that dropped it must fail here, not at run time)
    # and SpliceVault (optional at run time, but its CODE must be here or the plugin silently no-ops)
    for p in LoF CADD dbNSFP SpliceAI REVEL AlphaMissense NMD SpliceVault; do test -f "/plugins/$p.pm" || { echo "MISSING plugin code: /plugins/$p.pm" >&2; exit 1; }; done; \
