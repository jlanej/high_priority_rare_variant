...
- **PMS2 / pseudogene-aware calling is a known blind spot, not yet mitigated.** The PMS2/PMS2CL paralogy makes short-read PMS2 calls (and other segmental-duplication genes — CYP21A2, SMN1/2, NEB, GBA) low-confidence. These regions are currently **neither flagged nor masked** by any step (the `overlays.pediatric_cancer.flag_pseudogene_regions` config key is `[reserved]` and unread); region-flagging for review, then robust masking / paralog-aware calling, are future work.
