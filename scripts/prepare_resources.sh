...
do_verify() {
    ...
    verify_extra "$SPLICEAI_INDEL_OUT" spliceai_indel
...
do_emit() {
    local w=/dev/stdout; [[ -n "$EMIT_OUT" ]] && w="$EMIT_OUT"
    {
        ...
        echo "# still has a key for. gnomAD/ClinVar/dbNSFP/LOFTEE exports stay REMOVED — nothing reads"
        echo "# them. SpliceAI IS wired now (VEP plugin), so its paths are emitted; the pipeline skips"
        echo "# the splice keep-path gracefully if the files are not actually present."
